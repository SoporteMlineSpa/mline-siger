...
";
    ...
    public function generarPdf()
    {
        ...
        return $folios->map(function($folio, $index) use ($productos) {
            $folio = str_pad($folio, 10, "0", STR_PAD_LEFT);
            $fecha = date("Y-m-d");
            $rutReceptor = str_replace(".", "", strtoupper($this->centro->empresa->rut));
            $razonSocialReceptor = $this->centro->empresa->razon_social;
            ...
            $neto = round($montoTotal - ($montoTotal * ($tasaIva / 100)));
            $exento = 1;
            $producto = $productos->get($index)->map(function ($producto, $index) {
                // $numLinea debe tener un largo de 4 caracteres
                // $numLineaSII debe tener un largo de 4 caracteres
                // $sku debe tener un largo de 35 caracteres (rellenar al final)
                // $detalle debe tener un largo de 80 caracteres (rellenar al final)
            ...
            $data = [
                "Encabezado" => [
                    ...
                    "Emisor" => [
                        "RUTEmisor" => "96651910-K",
                        "RznSoc" => "COMPASS CATERING SA",
                        "GiroEmis" => "Servicios de Alimentacion",
                        "Acteco" => 726000,
                        "DirOrigen" => "AV. DEL VALLE N° 787, OF. 501",
                        "CmnaOrigen" => "Santiago",
                    ],
                    "Receptor" => [
                    ...
                    "Transporte" => [
                        "Chofer" => [
                            "RUTChofer" => $transporteRut,
                            "NombreChofer" => $transporteNombre
                        ],
                        "DirDest" => $direccionDestino,
                        "CmnaDest" => $comunaDestino
                    ]
                ],
                "Detalle" => $producto->toArray()
            ];
            $pdf = new \App\Helper\Dte(false);
            $pdf->setFooterText();
            $pdf->setLogo(public_path('img/logocompassc1.png')); // debe ser PNG!
            $pdf->agregar($data);
            $pdf->Output(public_path("storage/OUTPDF/R{$rutReceptor}T52F{$folio}.pdf"), 'F');

            return public_path("storage/OUTPDF/R{$rutReceptor}T52F{$folio}.pdf");
        });
    }
